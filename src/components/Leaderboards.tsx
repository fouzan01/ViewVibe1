...
import { Crown, Trophy, MapPin, ChevronDown } from 'lucide-react';
...
const Leaderboards: React.FC<LeaderboardsProps> = ({
  ...
  const { topUsers, userRank } = useMemo(() => {
    ...
    if (filterDistrict !== 'All') {
      filtered = filtered.filter(u => u.district === filterDistrict);
    }

    // 2. Sort by Timeframe Value
    const sorted = filtered.sort((a, b) => getSortValue(b) - getSortValue(a));

    // 3. Calculate Top 10
    const top10 = sorted.slice(0, 10).map((u, index) => ({
      ...u,
      rank: index + 1,
      displayValue: getSortValue(u)
    }));

    // 4. Calculate Current User Rank in this context
    let currentRank = null;
    if (currentUser) {
      const index = sorted.findIndex(u => u.id === currentUser.uid);
      if (index !== -1) {
        currentRank = index + 1;
      }
    }

    return { topUsers: top10, userRank: currentRank };
  }, [allUsers, leaderboardCategory, timeframe, filterState, filterDistrict, currentUser]);

  return (
    <div className="space-y-4">
      {/* Regional Glory Filters */}
      <div className="glass-card p-4 sm:p-6 border-white/5 bg-white/5 space-y-4">
        <div className="flex items-center gap-2 mb-2">
          <MapPin size={16} className="text-orange-500" />
          <h3 className="text-xs font-black uppercase tracking-[0.2em] text-white">Regional Glory Filters</h3>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">State / Region</label>
            <div className="relative group">
              <select
                ...
                className="w-full bg-black/40 border border-white/10 rounded-xl py-3 pl-4 pr-10 text-xs font-bold text-white focus:outline-none focus:border-orange-500/50 transition-all appearance-none cursor-pointer"
              >
                <option value="All">Global / All India</option>
                {Object.keys(indiaData).sort().map(s => (
              ...
              >
                ...
                <div className="relative group">
                  ...
                  >
                    ...
                    {indiaData[filterState]?.sort().map(d => (
      ...
      {topUsers.length === 0 ? (
        <div className="glass-card p-12 text-center space-y-4">
          ...
          <p className="text-slate-500 font-bold uppercase tracking-widest text-xs">No legends found in this territory yet.</p>
        </div>
      ) : (
        topUsers.map((item) => {
